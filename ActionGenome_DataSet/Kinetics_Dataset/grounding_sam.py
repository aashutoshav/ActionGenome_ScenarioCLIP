...
from dataloaders.kinetics_dataset import custom_collate_kineticswDino, KineticswDinoResp_Dataset
...
class GroundedSam:
    ...
    def create_dataset(self, args):
        ...
        if 'kinetics' in args.root_dir.lower():
            image_dataset = KineticswDinoResp_Dataset(
            ...
            image_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=custom_collate_kineticswDino,
            )
    ...
    def generate_response(self, args, image_loader=None):
        ...
        os.makedirs(os.path.join(args.storage_dir, "sam_results"), exist_ok=True)
            ...
            for img_file, batch_images, _, dino_annotations in pbar:
                ...
                try:
                    ...
                    for b in range(len(batch_images)):
                        ...
                        action_caption = img_file[b].split("/")[-3]
                        sub_folder = img_file[b].split("/")[-2]

                        os.makedirs(
                            os.path.join(args.storage_dir, "sam_results", action_caption), exist_ok=True
                        )
                        os.makedirs(
                            os.path.join(args.storage_dir, "sam_results", action_caption, sub_folder), exist_ok=True
                        )

                        npz_path = os.path.join(args.storage_dir, "sam_results", action_caption, sub_folder, os.path.basename(img_file[b]).replace(".jpg", "_grounding_sam.npz"))
                        ...
                        if is_json_file_empty(llama_path) or is_json_file_empty(dino_path):
                            print(
                        ...
                        self.generate_relations(
                            ...
                            npz_path,
                            dino_path,
                            npz_path.replace("_grounding_sam.npz", "_relations.jpg"),
                            llama_path,
                        )
    ...
    def show_masks_on_image(self, raw_image, masks, scores, labels, output_path):
        ...
        for i, label in enumerate(labels):
            ...
            #     / 255.0
            # )
            color = np.array([0.8, 0.8, 0.8, 0.8])
            ...
            if len(relation) == 3:
                ...
                if len(indices_o1) > 0 and len(indices_o2) > 0:
                    ...
                    if closest_indices:
                        ...
                        if o1_idx >= len(selected_masks) or o2_idx >= len(
                            selected_masks
                        ):
                            continue
                        ...
                        # print(
                        #     f"Generated focus image between {o1} and {o2} saved at {relation_output_path}"
                        # )
...
def main():
    ...
    parser.add_argument("--end_chunk", type=int, default=0, help="Max 400")
