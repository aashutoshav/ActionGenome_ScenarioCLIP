...
from dataloaders.kinetics_dataset import (
    custom_collate_kineticswLLaVa,
    KineticswLLaVAResp_Dataset,
)
...
class ObjDet_GroundingDINO:
    ...
    def create_dataset(self, args):
        ...
        if 'kinetics' in args.root_dir.lower():
            image_dataset = KineticswLLaVAResp_Dataset(
            ...
            image_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=custom_collate_kineticswLLaVa,
            )
    ...
    def detect_objects(self, args, image_loader=None):
        ...
        with tqdm(image_loader) as pbar:
            for img_files, batch_images, action_caption, llava_annotations in pbar:
                ...
                for i in range(batch):
                    ...
                    os.makedirs(os.path.join(args.storage_dir, "dino_results", action_caption[i]), exist_ok=True)
                    os.makedirs(os.path.join(args.storage_dir, "dino_results", action_caption[i], img_files[i].split('/')[-2]), exist_ok=True)
                    json_path = os.path.join(
                        args.storage_dir,
                        "dino_results",
                        action_caption[i],
                        img_files[i].split('/')[-2],
                        img_name.replace(".png", "_grounding_dino.json") if "png" in img_name else img_name.replace(".jpg", "_grounding_dino.json"),    
                    )
                ...
                if args.img_save:
                    for b in range(len(batch_images)):
                        ...
                        os.makedirs(
                            os.path.join(args.storage_dir, "bbox_visualised", img_files[b].split('/')[-3], img_files[b].split('/')[-2]),
                            exist_ok=True,
                        )
                        impath = os.path.join(
                            args.storage_dir,
                ...
                for i in range(batch):
                    ...
                    action_caption = img_files[i].split('/')[-3]
                    sub_folder = img_files[i].split('/')[-2]
                    os.makedirs(os.path.join(args.storage_dir, "dino_results", action_caption), exist_ok=True)
                    os.makedirs(os.path.join(args.storage_dir, "dino_results", action_caption, sub_folder), exist_ok=True)
                    img_name = os.path.basename(img_files[i])
                    json_path = os.path.join(
                        args.storage_dir,
                        "dino_results",
                        action_caption,
                        sub_folder,
                        img_name.replace(".jpg", "_grounding_dino.json"),
                    )
...
def main():
    ...
    parser.add_argument(
        "--color_mapping_json",
        type=str,
        default="/home/saali/Workbenches/Aashutosh/ActionGenome_DataSet/color_mapping.json",
    )
    parser.add_argument(
        "--start_chunk",
        type=int,
