...
class VanillaDataset(Dataset):
    ...
    def load_image_data_in_chunks(self):
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = []
            filenames = ['vanilla.jpg']
            futures.extend(
                executor.submit(self.process_chunk, "", filename)
...
class VanillaDatasetwLLaVa(Dataset):
    def __init__(
        self,
        root_dir,
        chunk_json_path,
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            futures.extend(
                [
                    executor.submit(self.collect_files, "", filename)
                    for filename in filenames
                ]
            )
    ...
    def collect_files(self, chunk_name, filename):
        img_file = os.path.join(self.root_dir, filename)
...
class VanillawDinoResp_Dataset(Dataset):
    def __init__(
        self,
        root_dir,
        chunk_json_path,
