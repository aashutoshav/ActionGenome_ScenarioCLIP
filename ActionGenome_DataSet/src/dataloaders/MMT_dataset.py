...
class MMT_Dataset(Dataset):
    def __init__(
        ...
        self.chunk_list = self.read_categories(
            os.path.join(self.root_dir, "moments_categories.txt")
        )
        self.chunk_list = self.chunk_list[start_chunk : min(end_chunk + 1, 292)]
    ...
    def read_categories(self, filepath):
        ...
        with open(filepath, "r") as f:
            moments_categories = f.read().splitlines()
            results = [x.split(",")[0] for x in moments_categories]
    ...
    def load_image_data_in_chunks(self):
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            for chunk_name in self.chunk_list:
                chunk_path = os.path.join(self.root_dir, "videos", chunk_name)
    ...
    def collect_files(self, sub_chunk_path, filename, chunk_name):
        img_file = os.path.join(sub_chunk_path, filename)
...
class MMTwLLaVAResp_Dataset(Dataset):
    def __init__(
        self,
        root_dir,
        start_chunk,
        ...
        """
        Args:
            Chunks start from 0 and go to 364 inclusive
        """
        self.root_dir = root_dir
        self.image_shape = image_shape
        self.transform = transform or transforms.Compose(
    ...
    def collect_files(self, sub_chunk_path, filename, chunk_name):
        ...
        gemmaResp_file = os.path.join(
            ...
            "gemma_jsons",
            chunk_name,
            os.path.basename(sub_chunk_path),
...
class MMTwDinoResp_Dataset(Dataset):
    def __init__(
        self,
        root_dir,
        start_chunk,
