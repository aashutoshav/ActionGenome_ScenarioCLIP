...
from dataloaders.vanilla_dataset import VanillaDataset
...
class VQA_LLaVA:
    ...
    def generate_response(self, args, image_loader=None):
        ...
        for img_file, batch_images, action_class in tqdm(image_loader):
            ...
            for i in range(batch):
                try:
                    ...
                    json_path = os.path.join(
                        args.storage_dir, "gemma_jsons", imgname.replace(".jpg", "_gemma.json")
                    )

                    if "multi" in args.root_dir.lower():
                        mmt_chunk = img_file[i].split('/')[-3]
                        mmt_sub_chunk = img_file[i].split('/')[-2]
                        os.makedirs(os.path.join(args.storage_dir, "gemma_jsons", mmt_chunk, mmt_sub_chunk), exist_ok=True)
                        json_path = os.path.join(
                            args.storage_dir, "gemma_jsons", mmt_chunk, mmt_sub_chunk, imgname.replace(".png", "_gemma.json")
                        )
                    ...
                    if "mmt" in args.root_dir.lower():
                        response_dict['response']['action'] = action_class[i]
...
def main():
    ...
    parser.add_argument(
        "--root_dir",
        type=str,
        default="/mnt/MIG_archive/Datasets/mmt/Multi_Moments_in_Time/",
    )
    parser.add_argument(
        "--storage_dir",
        type=str,
        default="/mnt/MIG_archive/Datasets/mmt/mmt_results",
    )
    parser.add_argument(
        "--chunk_json_path",
        type=str,
        default="/home/zeta/Workbenches/ActionGenome_DataSet/dataset/val_chunks.json",
    )
    parser.add_argument(
        "--metadata_json_path",
        type=str,
        default="/home/zeta/Workbenches/ActionGenome_DataSet/dataset/val_details.json",
    )
    parser.add_argument("--batch_size", type=int, default=16)
    parser.add_argument(
        "--cache_dir",
        default="/home/zeta/.cache/",
    )
    parser.add_argument("--max_workers", type=int, default=8)
    parser.add_argument("--start_chunk", type=int, default=0)
    parser.add_argument("--end_chunk", type=int, default=29)
    parser.add_argument('--gpu', type=str, default='0')
    parser.add_argument('--split', type=str, default='val', help='train/val for coco')
