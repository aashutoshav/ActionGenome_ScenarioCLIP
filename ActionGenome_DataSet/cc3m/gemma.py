        ...
        for img_file, batch_images, chunk_name in tqdm(image_loader):
            ...
            for i in range(batch):
                try:
                    ...
                    os.makedirs(os.path.join(args.storage_dir, "gemma_jsons", chunk_name[i]), exist_ok=True)
                    json_path = os.path.join(
                        args.storage_dir, "gemma_jsons", chunk_name[i], imgname.replace('.jpg', '_gemma.json')
                    )
...
def main():
    ...
    parser.add_argument(
        "--storage_dir",
        type=str,
        default="/mnt/MIG_archive/Datasets/CC3M_Train_Results",
    )
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument(
        "--cache_dir",
        default="/home/zeta/Workbenches/.cache",
    )
