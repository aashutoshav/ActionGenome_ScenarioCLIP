...
from dataloaders.kitti_dataset import kitti_Dataset, custom_collate_kitti
...
class VQA_LLaVA:
    ...
    def create_dataset(self, args):
        ...
        if "kitti" in args.root_dir.lower():
            ...
            image_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=custom_collate_kitti,
            )
    ...
    def generate_response(self, args, image_loader=None):
        ...
        for img_file, batch_images, chunk_name in tqdm(image_loader):
            ...
            for i in range(batch):
                try:
                    ...
                    os.makedirs(os.path.join(args.storage_dir, "gemma_jsons"), exist_ok=True)
                    os.makedirs(
                        os.path.join(args.storage_dir, "gemma_jsons", chunk_name[i]), exist_ok=True
                    )
                    json_path = os.path.join(
                        args.storage_dir, "gemma_jsons", chunk_name[i], imgname.replace(".png", "_gemma.json")
                    )
...
def main():
    ...
    parser.add_argument("--end_chunk", type=int, default=364)
    parser.add_argument('--gpu', type=str, default='0')
    parser.add_argument('--split', type=str, default='train', help='train/val for coco')
