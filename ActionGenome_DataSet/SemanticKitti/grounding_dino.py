...
from dataloaders.kitti_dataset import (
    kittiwLLaVAResp_Dataset,
    custom_collate_kittiwLLaVa,
)
...
class ObjDet_GroundingDINO:
    ...
    def create_dataset(self, args):
        ...
        if "kitti" in args.root_dir.lower():
            image_dataset = kittiwLLaVAResp_Dataset(
            ...
            image_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=custom_collate_kittiwLLaVa,
            )
    ...
    def detect_objects(self, args, image_loader=None):
        ...
        with tqdm(image_loader) as pbar:
            for img_files, batch_images, chunk_name, llava_annotations in pbar:
                ...
                for i in range(batch):
                    ...
                    if "kitti" in args.root_dir.lower():
                        os.makedirs(
                            os.path.join(
                                args.storage_dir, "dino_results", chunk_name[i]
                            ),
                            exist_ok=True,
                        )
                        basename = os.path.basename(img_files[i])
                        json_path = os.path.join(
                            args.storage_dir, "dino_results", chunk_name[i], basename.replace('.png', '_grounding_dino.json')
                        )
