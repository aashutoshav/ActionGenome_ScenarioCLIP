...
from dataloaders.kitti_dataset import kittiwDinoResp_Dataset, custom_collate_kittiwDino
...
class GroundedSam:
    ...
    def create_dataset(self, args):
        if "kitti" in args.root_dir.lower():
            image_dataset = kittiwDinoResp_Dataset(
            ...
            image_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=custom_collate_kittiwDino,
            )
            ...
            for img_file, batch_images, _, dino_annotations in pbar:
                    ...
                    for file in img_file:
                        if not os.path.exists(
                            os.path.join(
                                ...
                                "sam_results",
                                file.split("/")[-2],
                                os.path.basename(file).replace(".png", "_grounding_sam.npz")
                            )
                        ):
                            check = True
                            break

                if "kitti" in args.root_dir.lower() and not check:
                ...
                try:
                    ...
                    for b in range(len(batch_images)):
                        ...
                        os.makedirs(os.path.join(args.storage_dir, "sam_results", img_file[b].split("/")[-2]), exist_ok=True)
                        ...
                        llama_path = os.path.join(args.storage_dir, "gemma_jsons", img_file[b].split("/")[-2], os.path.basename(img_file[b]).replace(".png", "_gemma.json"))
                        dino_path = os.path.join(args.storage_dir, "dino_results", img_file[b].split("/")[-2], os.path.basename(img_file[b]).replace(".png", "_grounding_dino.json"))
...
def main():
    ...
    parser.add_argument(
        "--root_dir",
        type=str,
        default="/mnt/MIG_store/Datasets/KITTI2015/testing",
    )
    parser.add_argument(
        "--storage_dir",
        type=str,
        default="/mnt/MIG_store/Datasets/KITTI2015/kitti_test_results",
    )
    ...
    parser.add_argument(
        "--cache_dir",
        default="/home/zeta/Workbenches/ActionGenome_ScenarioCLIP/ActionGenome_DataSet/.cache",
